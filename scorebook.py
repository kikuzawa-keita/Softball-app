...
def show():
    ...
    with tab_bat:
        ...
        # 超詳細版のロジック（if input_precision == "超詳細版"）を完全に削除
    ...
    if st.button(f"{'上書き' if is_edit_mode else '新規'}保存", type="primary", use_container_width=True, disabled=is_game_limit_reached):
